...
function providerStatuses() {
  return [
    ...
    providerStatus("tts", "真实 TTS", Boolean(config.ttsProvider), "未选择 TTS_PROVIDER，当前只能使用浏览器语音回退", "fallback", ["TTS_PROVIDER", "FISH_AUDIO_API_KEY", "FISH_AUDIO_VOICE_ID"]),
    providerStatus("asr", "语音输入", Boolean(config.asrProvider), "未选择 ASR_PROVIDER，当前只能使用浏览器能力", "fallback", ["ASR_PROVIDER", "WHISPER_API_KEY"]),
    providerStatus("weather", "天气", Boolean(config.openWeatherApiKey && config.weatherCity), "缺少 OPENWEATHER_API_KEY 或 WEATHER_CITY", undefined, ["WEATHER_PROVIDER", "OPENWEATHER_API_KEY", "WEATHER_CITY"]),
    providerStatus("calendar", "日历/日程", false, "尚未选择日历接入方式", undefined, ["CALENDAR_PROVIDER"]),
    providerStatus("upnp", "家庭音响/UPnP", config.upnpEnabled === "true" && Boolean(config.upnpDeviceName), "缺少 UPNP_ENABLED=true 或 UPNP_DEVICE_NAME", undefined, ["UPNP_ENABLED", "UPNP_DEVICE_NAME"])
  ];
}
