import type { State, Track } from "../../shared/src/index.js";
...
export function routineForHour(hour: number): RoutineSlot {
  const normalized = Number(hour);
  return routineSlots.find((slot) => slot.hours.some((slotHour) => slotHour === normalized)) || routineSlots[4];
}

function scoreTrack(track: Track, slot: RoutineSlot, profile: ReturnType<typeof buildTasteProfile>, state: Partial<State> = {}) {
  const tags = new Set(track.tags || []);
  const liked = new Set(state.liked || []);
  const skipped = new Set(state.skipped || []);
  const profileTags = new Map(profile.topTags.map((item) => [typeof item === "string" ? item : item.tag, typeof item === "string" ? 1 : item.count]));
...
export function buildRadioPlan({
  tracks = [],
  state = {},
  hour,
  count = 6
  ...
  const queue = [...tracks]
    ...
    .slice(0, count)
    .map((item) => item.track);
  const topTags = profile.topTags.slice(0, 3).map((item) => (typeof item === "string" ? item : item.tag));
